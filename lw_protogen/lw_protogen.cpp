...
{
	cout << "Generating makefiles" << endl;
	...
	for (vector<pair<string, map<string, Package> > >::const_iterator package_group = package_groups.begin(); package_group != package_groups.end(); ++package_group) {
		cout << "  Generating makefile for package group " << package_group->first << endl;
		ofstream mk((target_dir + "/Packages/" + first_upper(package_group->first) + "/Makefile.am").c_str());
		if (!mk.is_open()){
			cout << "  !!! Couldn't create Makefile !!! " << endl;
			exit(1);
		}
	...
	{
		cout << " Generating makefile for enums" << endl;
		ofstream mk((target_dir + "/Enums/Makefile.am").c_str());
		if (!mk.is_open()){
			cout << "   !!!! Couldn't create makefile for enums" << endl;
			exit(1);
		}
		mk << "noinst_LTLIBRARIES = libProtocolEnums.la" << endl;
		string enum_src;
		for (vector<Enum>::const_iterator en = enums.begin(); en != enums.end(); ++en) {
			enum_src += make_class_name(en->name) + ".h ";
		}
		for (vector<pair<string, map<string, Package> > >::const_iterator pg = package_groups.begin(); pg != package_groups.end(); ++pg) {
			enum_src += first_upper(pg->first) + "Packages.h ";
		}
		mk << "libProtocolEnums_la_SOURCES = " << enum_src << endl;
		mk << "INCLUDES=-I$(top_srcdir)/src" << endl;
	}
	{
		cout << " Generating global Makefile" << endl;
		ofstream mk((target_dir + "/Makefile.am").c_str());
		if (!mk.is_open()){
		...
		mk << "libProtocol_la_SOURCES = " << src << endl;
		mk << "INCLUDES=-I$(top_srcdir)/src" << endl;
